 ...
 * Prof.: Hugo Escalpelo
 * Modificado por: Omar Abundis (Alumno)
...
void setup() 
{
  ...
  Serial.println(WiFi.localIP());
                                                                     ...
                                                                     // Si se logró la conexión, encender led
  if (WiFi.status () > 0)
  {
  digitalWrite (ledPin, HIGH);
  }
...
void callback(char* topic, byte* message, unsigned int length) 
{
  ...
  if (String(topic) == "codigoIoT/detectorSintomas/esp") 
  {                                                                         // En caso de recibirse mensaje en el tema codigoIoT/detectorSintomas/esp
    if(messageTemp == "true")
    {
